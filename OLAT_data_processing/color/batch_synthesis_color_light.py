import os
from synthesis_color_light import OLATRelightAndComposite, infer_out_dir
import argparse
...
def batch_process(root_dir, root_alpha, root_out,
                  olat_txt,
                  base_map_dir,
                  envmap_dir,
                  background_dir,
    ...
    for id_name in os.listdir(root_dir):
        ...
        for cam_idx in range(cam_range[0], cam_range[1] + 1):
            ...
            # 推导输出目录
            # out_dir = infer_out_dir(cam_path)
...
def process_first_session_c11_all_ids(root_dir, root_alpha, root_out,
                                      olat_txt,
                                      base_map_dir,
                                      envmap_dir,
                                      background_dir):
    """
    遍历 root_dir 下的所有 ID
    每个 ID -> 第一个 session -> 只处理 C11
    """
    for id_name in sorted(os.listdir(root_dir)):
        id_path = os.path.join(root_dir, id_name)
        if not os.path.isdir(id_path):
            continue

        # 找到该 ID 下的所有 session
        sessions = [s for s in os.listdir(id_path) if os.path.isdir(os.path.join(id_path, s))]
        if not sessions:
            print(f"[WARN] ID {id_name} 下没有 session，跳过")
            continue

        # 取第一个 session（按字典序）
        first_session = sorted(sessions)[0]
        session_path = os.path.join(id_path, first_session)

        cam_path = os.path.join(session_path, "C11")
        if not os.path.isdir(cam_path):
            print(f"[WARN] {id_name} 下的 {first_session}/C11 不存在，跳过")
            continue
        ...
        relighter.run()
        print(f"✅ 完成 {id_name} 的 {first_session}/C11")

import random

def batch_process_random_envmaps(root_dir, root_alpha, root_out,
                                 olat_txt,
                                 base_map_dir,
                                 envmap_dir,
                                 background_dir,
...
def process_one(cam_path, root_dir, root_alpha, root_out, olat_txt, base_map_dir, envmaps, background_dir):
    """单个相机目录的处理逻辑"""
...
def batch_process_parallel(root_dir, root_alpha, root_out,
                           olat_txt,
                           base_map_dir,
                           envmap_dir,
                           background_dir,
    ...
    for id_name in os.listdir(root_dir):
        # print(int(id_name))
        # if int(id_name) < 86 or int(id_name) > 135:
        #     continue
        id_path = os.path.join(root_dir, id_name)
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        for cam_path in cam_paths:
            ...
            futures.append(executor.submit(
                process_one, cam_path, root_dir, root_alpha, root_out, olat_txt, base_map_dir, envmaps, background_dir
            ))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root_dir", required=True, help="Step1 输出目录 (ori_OLAT_x)")
    parser.add_argument("--root_alpha", required=True, help="Step3 输出目录 (alpha_data_x)")
    parser.add_argument("--root_out", required=True, help="Step4 输出目录 (synthetic_light_x)")
    ...
    parser.add_argument("--num_envmaps", type=int, default=10)
